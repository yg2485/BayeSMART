...
Rcpp::List BayeSMART(arma::mat V, arma::mat Y, arma::mat P,  int K, NumericVector e, double f, 
                   NumericVector eta, double tau, double alpha_gamma, NumericMatrix beta_gamma, NumericVector mu_initial, 
                   NumericVector sigma_initial, NumericVector alpha, NumericVector omega_initial, arma::mat Z_initial, NumericVector weight, double lambda);
...
arma::mat rinvwish(double v, arma::mat S, double lambda);
arma::vec mvrnorm(vec mu, mat Sigma);


// [[Rcpp::export]]
Rcpp::List BayeSMART(arma::mat V, arma::mat Y, arma::mat P,  int K, NumericVector e, NumericVector f, 
                   ...
                   NumericVector omega_initial, arma::mat Z_initial, NumericVector weight, double lambda) {
  ...
  for(int t = 0; t < iter; t++)
  {
    ...
    for(q = 0; q < K; q++){
      ...
      for (qq = 0; qq < Y_dim; qq++){
        for (qqq = 0; qqq < Y_dim; qqq++){
          ...
          // if (qq == qqq){
          //   beta_gamma_new(qq, qqq) += 0.000000000000000001;
          // }
        }
      }
      arma::mat new_sigma = rinvwish(alpha_gamma_new, beta_gamma_new, lambda);
      for (qq = 0; qq < Y_dim; qq++){
        for (qqq = 0; qqq < Y_dim; qqq++){
    ...
    // Rcout <<t<< "done with update sigma\n";
    ...
    // Rcout <<t<< "done with update omega\n";
    ...
    for (h = 0; h < N; h++){
      ...
      for(q = 0; q < K; q++)
      {
        ...
        double aaa = arma::det(new_sigma);
        if (aaa > 0){
          prob_temp(q) -= 0.5*log(aaa);
        }else{
          prob_temp(q) -= 0.5*log(0.000000000001);
        }
        // Rcout << aaa << " ";
        // prob_temp(q) -= 0.5*log(arma::det(new_sigma) + 0.000000000000001);
        ...
        for (qq = 0; qq < V_dim; qq++){
          prob_temp(q) = prob_temp(q) + weight(h)*V(h, qq)*log(omega(q, qq) + 0.0000000000001);
        }
...
bool isNearlySingular(const arma::mat& M, double tolerance = 1e-12) {
  if (M.has_nan() || M.has_inf()) {
    return true;  // Considered as singular for safety
  }
  double cond = arma::cond(M);
  if (cond > 1e15 || std::isinf(cond)) {  // Adjust threshold as needed
    return true;
  }
  // vec s = svd(M);
  // double smallest_singular_value = s.min();
  // double largest_singular_value = s.max();
  // 
  // return smallest_singular_value < tolerance || (largest_singular_value / smallest_singular_value) > 1.0 / tolerance;
  arma::vec s = svd(M);
  if (s.is_empty() || s.min() < tolerance) {
    return true;
  }
...
arma::mat rinvwish(double v, arma::mat S, double lambda){
  ...
  arma::mat LA = trimatl(L) * trimatl(A);
  // arma::mat regularizedLA;
  // if (isNearlySingular(LA)) {
  //   regularizedLA = LA + lambda * arma::eye<arma::mat>(LA.n_rows, LA.n_cols);
  // } else {
  //   regularizedLA = LA;
  // }
  arma::mat regularizedLA = LA + lambda * arma::eye<arma::mat>(LA.n_rows, LA.n_cols);
  arma::mat LA_inv = pinv(regularizedLA);
  // arma::mat LA_inv = pinv(trimatl(trimatl(L) * trimatl(A)));
...
// mat rwishart(double v, const mat& S) {
//   int p = S.n_rows;
//   mat Z = zeros(p, p);
//   
//   for(int i = 0; i < p; ++i) {
//     Z(i, i) = sqrt(R::rchisq(v - i));  // Diagonal elements from Chi-squared distribution
//   }
//   
//   for(int i = 1; i < p; ++i) {
//     for(int j = 0; j < i; ++j) {
//       Z(i, j) = R::rnorm(0.0, 1.0);  // Lower triangle from normal distribution
//     }
//   }
//   
//   mat L = chol(S, "lower");
//   mat A = L * Z;
//   
//   return A * A.t();  // Wishart-distributed matrix
// }
// 
// // [[Rcpp::export]]
// mat rinvwish(double v, arma::mat S) {
//   double lambda = 1e-3;
//   mat W = rwishart(v, S);
//   mat W1 = W + lambda * arma::eye<arma::mat>(W.n_rows, W.n_cols);
//   return pinv(W1);  // Inverse of the Wishart matrix
// }
