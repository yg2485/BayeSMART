...
# n_cell_type <- length(unique(cell_info$name))
...
# Plot the posterior cell type composition result
omega <- apply(result$omega, c(2, 3), mean)
colnames(omega) <- c("dead", "tumor", "connective", "inflamm", "no label", "epithelial")
rownames(omega) <- c("domain 1", "domain 2", "domain 3", "domain 4", "domain 5", "domain 6")


par(mfrow = c(1,1))
groups <- c("dead", "tumor", "connective", "inflamm", "no label", "epithelial")
colors <- c("#FFFF99","#FF6666", "#3399FF", "#66CC00", "#808080", "#FFB266")


barplot(t(omega), beside = TRUE, col = colors, 
        main = "Cell Type Composition of Domains",
        ylab = "Cell Type Proportion",
        legend.text = rownames(omega))


# Add a legend
legend("topright", title = "cell type", legend = colnames(omega), fill = colors)
...
# After manually switch the labels we save the result in "../data/each_sample"
# Plot the result for each sample
